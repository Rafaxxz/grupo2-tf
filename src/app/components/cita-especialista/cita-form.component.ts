...
export class CitaFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.id) {
      ...
      this.svc.getById(this.id).subscribe(d => {
        ...
        this.cita = {
          ...d,
          fechaHora: d.fechaHora ? d.fechaHora.substring(0, 16) : ''
        };
      });
    }
  }
  ...
  guardar() {
    ...
    // Convertir a ISO 8601 completo con offset local para que OffsetDateTime lo acepte.
    const fechaISO = new Date(this.cita.fechaHora).toISOString();
    ...
    const obs = this.editando
      ...
      : this.svc.insert(payload as any);

    obs.subscribe({
      next: () => this.router.navigate(['/citas']),
      error: (e: any) => {
        this.error = e.error?.message || 'Error al guardar la cita. Intenta de nuevo.';
